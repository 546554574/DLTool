...
class DLPermissionDialog : DLBaseDialog {
    ...
    lateinit var contentSpan: SpannableStringBuilder
    constructor(
        context: Context,
    ...
    private fun initContentSpan() {
        ...
        userSpan.setSpan(userClickableSpan, 0, userSpan.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        ...
        yinsiSpan.setSpan(clickableSpan, 0, yinsiSpan.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    override fun initEvent() {
       ...
       var contentTv = dialogView!!.findViewById<TextView>(R.id.contentTv)
        refundLv.setOnClickListener {
            if (onPermissionClickListener != null) {
                onPermissionClickListener!!.cancelClickListener()
            }
            dismiss()
            DLPermissionUtil.isShow = false
        }
        sureLv.setOnClickListener {
            DLSPTool.putInt(context, DLPermissionUtil.APP_FIRST, 1)
            if (onPermissionClickListener != null) {
